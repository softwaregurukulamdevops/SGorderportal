...
resource "azurerm_resource_group" "example" {
  name     = "rg-orderportal-resources"
  location = "Canada East"  # Replace with your desired location
}

# Define the Standard App Service Plan
resource "azurerm_app_service_plan" "standard_plan" {
  name                = "rg-orderportal-app-service-plan"
...
resource "azurerm_app_service" "code_service" {
  name                = "rg-orderportal-code-app-service"
  location            = azurerm_resource_group.example.location
  resource_group_name = azurerm_resource_group.example.name
